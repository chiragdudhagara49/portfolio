function Work() {
    return (
        <div className="bg-gray-900 text-white p-5 " >
            <div className="border rounded-xl border-slate-600">
                <div className="flex text-6xl p-16 font-kanit">Our Skills</div>
                <div className="grid grid-cols-7">
                    <div className=" m-3 pt-3 pb-3 pr-10 pl-10 rounded-lg shadow-sm shadow-white bg-gray-700 bg-opacity-30">
                        <p className="pb-[10px]">NodeJs</p>
                        <img className="h-[100px]" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original-wordmark.svg" />
                    </div>


                    <div className=" m-3 pt-3 pb-3 pr-10 pl-10 rounded-lg shadow-sm shadow-white  bg-gray-700 bg-opacity-30">
                        <p className="pb-[10px]">flutter</p>

                        <img className="h-[100px]" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/flutter/flutter-original.svg" />

                    </div>
                    <div className="m-3 pt-3 pb-3 pr-10 pl-10 rounded-lg shadow-sm shadow-white bg-gray-700 bg-opacity-30">
                        <p className="pb-[10px]">React</p>

                        <img className="h-[100px]" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original-wordmark.svg" />


                    </div>

                    <div className="m-3 pt-3 pb-3 pr-10 pl-10 rounded-lg shadow-sm shadow-white bg-gray-700 bg-opacity-30">
                        <p className="pb-[10px]">HTML</p>


                        <img className="h-[100px]" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original-wordmark.svg" />


                    </div>
                    <div className="m-3 pt-3 pb-3 pr-10 pl-10 rounded-lg shadow-sm shadow-white bg-gray-700 bg-opacity-30">
                        <p className="pb-[10px]">Css</p>

                        <img className="h-[100px]" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original-wordmark.svg" />

                    </div>

                    <div className="m-3 pt-3 pb-3 pr-10 pl-10 rounded-lg shadow-sm shadow-white bg-gray-700 bg-opacity-30">
                        <p className="pb-[10px]">javascript</p>

                        <img className="h-[100px]" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" />


                    </div>
                    <div className="m-3 pt-3 pb-3 pr-10 pl-10 rounded-lg shadow-sm shadow-white bg-gray-700 bg-opacity-30">
                        <p className="pb-[10px]">MongoDB</p>


                        <img className="h-[100px]" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-original-wordmark.svg" />


                    </div>

                    <div className="m-3 pt-3 pb-3 pr-10 pl-10 rounded-lg shadow-sm shadow-white bg-gray-700 bg-opacity-30">
                        <p className="pb-[10px]">Tailwindcss</p>


                        <img className="h-[100px]" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tailwindcss/tailwindcss-original-wordmark.svg" />


                    </div>

                    <div className="m-3 pt-3 pb-3 pr-10 pl-10 rounded-lg shadow-sm shadow-white bg-gray-700 bg-opacity-30">
                        <p className="pb-[10px]">Material UI</p>


                        <img className="h-[100px]" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/materialui/materialui-original.svg" />


                    </div>

                    <div className="m-3 pt-3 pb-3 pr-10 pl-10 rounded-lg shadow-sm shadow-white bg-gray-700 bg-opacity-30">
                        <p className="pb-[10px]">Next</p>


                        <img className="h-[100px]" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nextjs/nextjs-original-wordmark.svg" />


                    </div>
                </div>

                <div className="p-16">
                    <div className="text-6xl flex font-kanit">Our Portfolio</div>
                    <div className="flex">
                        <div className="h-[100px] w-[100px] bg-black m-5" >
                            <div>img of project</div>
                            <span>Name1 </span>
                        </div>
                        <div className="h-[100px] w-[100px] bg-black m-5" >
                            <div>img of project</div>
                            <span>Name2 </span>
                        </div>

                    </div>
                </div>
            </div>

        </div>

    );
}

export default Work;